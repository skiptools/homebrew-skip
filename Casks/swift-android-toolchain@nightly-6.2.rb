cask "swift-android-toolchain@nightly-6.2" do
  ...
  postflight do
    system_command "xattr", args: ["-d", "-r", "-s", "com.apple.quarantine", "#{staged_path}/#{artifact}"], must_succeed: true
    system_command "#{swiftcmd}", args: ["sdk", "install", "#{staged_path}/#{artifact}"], must_succeed: true
    system_command "#{sdkpath}/swift-android/scripts/setup-android-sdk.sh"], must_succeed: true
  end
